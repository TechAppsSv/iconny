...
function Navigation(options) {
    ...
    this.SVG_BACK = '<h3 class="fas fa-chevron-left" style="font-size:25px;"></h3>';
    this.SVG_FORWARD = '<h3 class="fas fa-chevron-right" style="font-size:25px;"></h3>';
    ...
    this.SVG_ADD = '<h3 class="fas fa-plus" style="font-size:20px;"></h3>';
    ...
    if (options.showUrlBar) {
        $('#nav-body-ctrls').append('<form id="formTask3"></form><div id="tasks3"></div></div> <div style="widht:20%;"></div> <a href="javascript:mostrar();" onclick="mostrar2()"><h1 style="font-size:25px;" class="fas fa-lock"></h1></a>  <li style="color:#1c1c1c;">.</li></div> <a href="javascript:mostrar3();" onclick="mostrar2()"><h1 style="font-size:25px;"class="fas fa-pager"></h1></a>  <script src="config/newtab.js"></script> <script src="config/config2.js"></script> <div class="sep" style="height: 50px; "></div><input id="nav-ctrls-url"  type="text" title="Enter an address or search term"/>')
    }
...
Navigation.prototype.newTab = function (url, options) {
    ...
    // add webview    
    let composedWebviewTag = `<webview class="nav-views-view active" preload="./contextual.js"  useragent=" Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.116 Safari/537.36 Edg/83.0.478.61
